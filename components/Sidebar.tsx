...
import { IoSearchOutline, IoCaretForwardCircleOutline } from "react-icons/io5";
    ...
    <aside className="fixed top-0 left-0 w-64 h-full bg-black">
      <div className="flex flex-col items-center h-full m-5 mt-5">
        ...
        <ul className="w-full mt-4">
          ...
          <Link href="/room/currentRoom">
            <a>
              <li
                className={`${
                  router.pathname.includes("/room") &&
                  !router.pathname.includes("currentRoom")
                    ? activeLink
                    : inactiveLink
                } flex items-center gap-3 p-2 text-sm rounded cursor-pointer  hover:text-white`}
              >
                <IoCaretForwardCircleOutline className="text-2xl" />
